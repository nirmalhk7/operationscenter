...
// On the local IP view, keeps only explicitly local tiles and removes empty groups.
(function filterLocalHomepage() {
  if (window.location.host !== "10.0.0.10") return;

  // Removes non-local service entries and then prunes any emptied service groups.
  const render = () => {
    // Iterates over every Homepage service list rendered by the client.
    Array.from(document.querySelectorAll(".services-list")).forEach((list) => {
      // Removes entries that are not tagged with the local-only id prefix.
