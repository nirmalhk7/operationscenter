...
test("MountainValue workflow is deterministic and trader-facing docs mention paper mode", () => {
  ...
  assert.match(lobsterPlugin, /name: "lobster"/u);
  assert.match(lobsterPlugin, /acquireWorkflowLock/u);
  assert.match(lobsterPlugin, /\.mountainvalue-workflow\.lock/u);
  assert.match(lobsterPlugin, /status: "SKIPPED"/u);
  ...
  assert.match(ansible, /MountainValue execute locked signal/u);
  assert.match(ansible, /MountainValue lock next-day signal/u);
  ...
  assert.match(ansible, /5,20,35,50 8-13 \* \* 1-5/u);
  assert.match(ansible, /20 14 \* \* 1-5/u);
